...
export default function DesktopIcon({ icon, label, onClick }) {
  return (
    <motion.div
      ...
      onClick={onClick}
    >
      <img src={icon} alt={label} className="w-10 h-10 sm:w-20 sm:h-20 hover:border-2 hover:border-green-300 hover:rounded-lg" />
      <span className="mt-1 text-black text-center text-sm sm:text-xl bg-green-300 px-2 sm:px-5">{label}</span>
    </motion.div>
  );
}
